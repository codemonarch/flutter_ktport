...
class KFMap<K, V> extends MapBase<K, V> {
  ...
  KFMap<K, V> takeUnless(bool block(KFMap<K, V> list)) =>
  ...
  // forEachEntry
  forEachEntry(void block(MapEntry<K, V> e)) =>
      _innerMap.forEach((k, v) => block(MapEntry(k, v)));
  ...
  add(Object obj) {
    var tmp =
        obj is Map<K, V> ? obj : (obj is KFMap<K, V> ? obj._innerMap : null);
  ...
  // filterTo
  KFMap<K, V> filterTo<M extends Map<K, V>>(
      M dest, bool block(MapEntry<K, V> e)) {
    forEachEntry((it) {
      if (block(it)) {
  ...
  // filterNotTo
  KFMap<K, V> filterNotTo<M extends Map<K, V>>(
      M dest, bool block(MapEntry<K, V> e)) {
    forEachEntry((it) {
      if (!block(it)) {
        dest[it.key] = it.value;
  ...
  // mapTo
  KFMap<K2, V2> mapTo<K2, V2, C extends Map<K2, V2>>(
      C dest, MapEntry<K2, V2> block(MapEntry<K, V> e)) {
    forEachEntry((it) {
      var item = block(it);
  ...
  // mapToListTo
  KFList<R> mapToListTo<R, C extends List<R>>(
      C dest, R block(MapEntry<K, V> e)) {
    forEachEntry((it) {
      dest.add(block(it));
    });
    return listOf(dest);
  ...
  // mapKeysTo
  KFMap<K2, V2> mapKeysTo<K2, V2, C extends Map<K2, V2>>(
      C dest, MapEntry<K2, V2> block(K k)) {
    forEachEntry((it) {
  ...
  // mapValuesTo
  KFMap<K2, V2> mapValuesTo<K2, V2, C extends Map<K2, V2>>(
      C dest, MapEntry<K2, V2> block(V v)) {
    forEachEntry((it) {
...
KFMap<K, V> mapOf<K, V>(Map<K, V> map) =>
    KFMap<K, V>().also((it) => it.addAll(map));

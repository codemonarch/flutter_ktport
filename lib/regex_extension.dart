...
bool isEmail(String str) =>
    regexMatch(str, "^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*\$");
bool isPhoneNumber(String str) =>
    regexMatch(str, "^(\\(\\d{3,4}-)|\\d{3.4}-)?\\d{7,8}\$");
bool isCellPhoneNumber(String str) => regexMatch(str,
    "^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\\d{8}\$");
bool isChinesePhoneNumber(String str) =>
    regexMatch(str, "\\d{3}-\\d{8}|\\d{4}-\\d{7}");
bool isIdCardNumber(String str) => regexMatch(str, "^\\d{15}|\\d{18}\$");
bool isShortIdCardNumber(String str) =>
    regexMatch(str, "^([0-9]){7,18}(x|X)?\$");
bool isUrl(String str) => regexMatch(str, "[a-zA-z]+://[^\\s]*");
bool isDomain(String str) => regexMatch(
    str, "[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(/.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+/.?");
bool isValidAccount(String str) =>
    regexMatch(str, "^[a-zA-Z][a-zA-Z0-9_]{5,31}\$");
bool isValidPassword(String str) => regexMatch(str, "^[a-zA-Z]\\w{5,31}\$");
bool isStrongPassword(String str) =>
    regexMatch(str, "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}\$");
bool isDate(String str) => regexMatch(str, "^\\d{4}-\\d{1,2}-\\d{1,2}");
bool isValidXml(String str) =>
    regexMatch(str, "^([a-zA-Z]+-?)+[a-zA-Z0-9]+\\\\.[x|X][m|M][l|L]\$");
bool isBlankLine(String str) => regexMatch(str, "\\n\\s*\\r");
bool isValidHtml(String str) =>
    regexMatch(str, "<(\\S*?)[^>]*>.*?</\\1>|<.*? />");
bool isValidQQNumber(String str) => regexMatch(str, "[1-9][0-9]{4,}");
bool isValidPostCode(String str) => regexMatch(str, "[1-9]\\d{5}(?!\\d)");
bool isValidIPAddress(String str) => regexMatch(str,

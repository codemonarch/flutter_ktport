...
class KFString {
  ...
  // origin
  Iterable<Match> allMatches(Object string, [int start = 0]) =>
      _innerString.allMatches(
          string is String
              ? string
              : (string is KFString ? string._innerString : ""),
          start);
  Match matchAsPrefix(Object string,
          [int start = 0]) =>
      _innerString.matchAsPrefix(
          string is String
              ? string
              : (string is KFString ? string._innerString : ""),
          start);
  ...
  bool operator ==(Object other) => other is String
      ? _innerString == other
      : (other is KFString ? _innerString == other._innerString : false);
  int compareTo(Object other) => _innerString.compareTo(
      other is String ? other : (other is KFString ? other._innerString : ""));
  bool endsWith(Object other) => _innerString.endsWith(
      other is String ? other : (other is KFString ? other._innerString : ""));
  bool startsWith(Pattern pattern, [int index = 0]) =>
      _innerString.startsWith(pattern, index);
  int indexOf(Pattern pattern, [int start]) => start == null
      ? _innerString.indexOf(pattern)
      : _innerString.indexOf(pattern, start);
  int lastIndexOf(Pattern pattern, [int start]) => start == null
      ? _innerString.lastIndexOf(pattern)
      : _innerString.lastIndexOf(pattern, start);
  bool get isEmpty => _innerString.isEmpty;
  bool get isNotEmpty => _innerString.isNotEmpty;
  KFString operator +(Object other) => KFString(_innerString +
      (other is String
          ? other
          : (other is KFString ? other._innerString : "")));
  KFString substring(int startIndex, [int endIndex]) =>
      KFString(endIndex == null
          ? _innerString.substring(startIndex)
          : _innerString.substring(startIndex, endIndex));
  ...
  KFString padLeft(int width, [Object padding = ' ']) =>
      KFString(_innerString.padLeft(
          width,
          padding is String
              ? padding
              : (padding is KFString ? padding._innerString : "")));
  KFString padRight(int width, [Object padding = ' ']) =>
      KFString(_innerString.padRight(
          width,
          padding is String
              ? padding
              : (padding is KFString ? padding._innerString : "")));
  bool contains(Pattern other, [int startIndex = 0]) =>
      _innerString.contains(other, startIndex);
  KFString replaceFirst(Pattern from, Object to, [int startIndex = 0]) =>
      KFString(_innerString.replaceFirst(
          from,
          to is String ? to : (to is KFString ? to._innerString : from),
          startIndex));
  KFString replaceFirstMapped(Pattern from, String replace(Match match),
          [int startIndex = 0]) =>
      KFString(_innerString.replaceFirstMapped(from, replace, startIndex));
  KFString replaceAll(Pattern from, Object replace) =>
      KFString(_innerString.replaceAll(
          from,
          replace is String
              ? replace
              : (replace is KFString ? replace._innerString : from)));
  KFString replaceAllMapped(Pattern from, String replace(Match match)) =>
      KFString(_innerString.replaceAllMapped(from, replace));
  KFString replaceRange(int start, int end, Object replacement) =>
      KFString(_innerString.replaceRange(
          start,
          end,
          replacement is String
              ? replacement
              : (replacement is KFString ? replacement._innerString : "")));
  KFList<KFString> split(Pattern pattern) =>
      listOf(_innerString.split(pattern).map((it) => KFString(it)).toList());
  KFString splitMapJoin(Pattern pattern,
          {String onMatch(Match match), String onNonMatch(String nonMatch)}) =>
      KFString(_innerString.splitMapJoin(pattern,
  ...
  KFString substringBefore(Pattern pattern) =>
      klet(indexOf(pattern), (idx) => idx == -1 ? this : substring(0, idx));
  KFString substringAfter(Pattern pattern) => klet(
      indexOf(pattern),
      (idx) => idx == -1
          ? this
          : substring(idx + pattern.toString().length, length));
  KFString substringBeforeLast(Pattern pattern) =>
      klet(lastIndexOf(pattern), (idx) => idx == -1 ? this : substring(0, idx));
  KFString substringAfterLast(Pattern pattern) => klet(
      lastIndexOf(pattern),
      (idx) => idx == -1
          ? this
          : substring(idx + pattern.toString().length, length));
  ...
  KFString removePrefix(Pattern pattern) =>
      startsWith(pattern) ? substring(pattern.toString().length, length) : this;
  KFString removeSuffix(Pattern pattern) => endsWith(pattern.toString())
      ? substring(0, length - pattern.toString().length)
      : this;
  KFString removeSurrounding(Pattern pattern) =>
      removePrefix(pattern).removeSuffix(pattern);
  KFString replaceBefore(Pattern pattern, Object replacement) => klet(
      indexOf(pattern),
      (idx) => idx == -1 ? this : replaceRange(0, idx, replacement));
  KFString replaceAfter(Pattern pattern, Object replacement) => klet(
      indexOf(pattern),
      (idx) => idx == -1
          ? this
          : replaceRange(idx + pattern.toString().length, length, replacement));
  KFString replaceAfterLast(Pattern pattern, Object replacement) => klet(
      lastIndexOf(pattern),
      (idx) => idx == -1
          ? this
          : replaceRange(idx + pattern.toString().length, length, replacement));
  KFString replaceBeforeLast(Pattern pattern, Object replacement) => klet(
      lastIndexOf(pattern),
      (idx) => idx == -1 ? this : replaceRange(0, idx, replacement));
  KFList<KFString> lines() =>
      listOf(_innerString.split("\n").map((it) => KFString(it)).toList());
  ...
  KFString getPathDirectory() => substringBeforeLast("/")
      .let((it) => it._innerString == "" ? stringOf("/") : it);

  // rarnu
  KFString toJsonEncoded() =>
  ...
  KFString extension() =>
      klet(indexOf("."), (idx) => idx == -1 ? "" : substringAfterLast("."));
  KFString replaceTag(String tag, KFString block()) => replaceAll(tag, block());
  KFString skipEmptyLine() =>
      lines().filterNot((it) => it.trim() == stringOf("")).joinToString("\n");
  KFPair<KFString, KFString> toPair() =>
      split("=").map((it) => it.trim()).let((it) => KFPair(it[0], it[1]));
  ...
  File asFileWriteText(Object s) {
    ...
    return kalso(
        File(_innerString),
        (ffile) => ffile.writeAsStringSync(
            s is String ? s : (s is KFString ? s._innerString : "")));
  }

  KFString asFileReadText() => klet(File(_innerString),
      (ffile) => KFString(ffile.existsSync() ? ffile.readAsStringSync() : ""));

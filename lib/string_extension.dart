...
class KFString {
  ...
  void forEach(void action(KFString s)) {
    for(int i = 0; i < length; i++) {
      action(KFString(_innerString[i]));
    }
  }
  void forEachIndexed(void action(int idx, KFString s)) {
    for(int i = 0; i < length; i++) {
  ...
  File asFileWriteText(Object s) {
    klet(Directory(substringBeforeLast("/")._innerString), (dfile) { if (!dfile.existsSync()) dfile.createSync(recursive: true); });
  ...
  asFileMkdirs() => klet(Directory(_innerString), (dir) { if (!dir.existsSync()) dir.createSync(recursive: true); });
